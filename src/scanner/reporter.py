...
class ReportGenerator:
    ...
    def _generate_html(self, results: dict, output_path: Optional[str] = None) -> str:
        ...
        for name, count in sorted(vuln_counts.items(), key=lambda x: -x[1]):
            ...
            chart_items += f"""
            <div class="chart-row" role="listitem" aria-label="{name}: {count} findings">
...
<body>
    <main class="container">
        ...
        <div style="background: var(--bg-card); border-radius: 12px; padding: 1.5rem; border: 1px solid var(--border);" role="list" aria-label="Vulnerabilities by type">
            {chart_items}
        </div>
