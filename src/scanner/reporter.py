...
class ReportGenerator:
    ...
    def _generate_html(self, results: dict, output_path: Optional[str] = None) -> str:
        ...
        chart_items = ""
        for i, (name, count) in enumerate(sorted(vuln_counts.items(), key=lambda x: -x[1])):
            max_count = max(vuln_counts.values()) if vuln_counts else 1
            width = (count / max_count) * 100
            chart_id = f"chart-label-{i}"
            chart_items += f"""
            <div class="chart-row">
                <span class="chart-label" id="{chart_id}">{name}</span>
                <div class="chart-bar-container" role="progressbar" aria-valuenow="{count}" aria-valuemin="0" aria-valuemax="{max_count}" aria-labelledby="{chart_id}">
