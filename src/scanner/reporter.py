...
class ReportGenerator:
    ...
    def _generate_html(self, results: dict, output_path: Optional[str] = None) -> str:
        ...
        chart_items = ""
        # Calculate max_count once outside the loop for performance
        max_count = max(vuln_counts.values()) if vuln_counts else 1
        for name, count in sorted(vuln_counts.items(), key=lambda x: -x[1]):
            width = (count / max_count) * 100
            # Add ARIA attributes to the bar chart container to make it accessible to screen readers
            chart_items += f"""
            <div class="chart-row">
                <span class="chart-label">{name}</span>
                <div class="chart-bar-container" role="progressbar" aria-valuenow="{count}" aria-valuemin="0" aria-valuemax="{max_count}" aria-label="{name}">
