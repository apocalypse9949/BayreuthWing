...
class ReportGenerator:
    ...
    def _generate_html(self, results: dict, output_path: Optional[str] = None) -> str:
        ...
        for name, count in sorted(vuln_counts.items(), key=lambda x: -x[1]):
            ...
            chart_items += f"""
            <div class="chart-row">
                <span class="chart-label" id="label-{safe_id}">{name}</span>
                <div class="chart-bar-container" role="progressbar" aria-valuenow="{count}" aria-valuemin="0" aria-valuemax="{max_count}" aria-labelledby="label-{safe_id}">
