...
class ReportGenerator:
    ...
    def _generate_html(self, results: dict, output_path: Optional[str] = None) -> str:
        ...
        for name, count in sorted(vuln_counts.items(), key=lambda x: -x[1]):
            ...
            chart_items += f"""
            <div class="chart-row">
                <span class="chart-label" id="label-{name.replace(' ', '-')}">{name}</span>
                <div class="chart-bar-container" role="progressbar" aria-labelledby="label-{name.replace(' ', '-')}" aria-valuenow="{count}" aria-valuemin="0" aria-valuemax="{max_count}">
...
<body>
    <main class="container">
        ...
        <section class="scan-info" aria-label="Scan Information">
            <div class="info-card">
        ...
        <section class="severity-grid" aria-label="Severity Summary">
        ...
        <section aria-labelledby="vuln-breakdown-title">
            <h2 id="vuln-breakdown-title" class="section-title">Vulnerability Breakdown</h2>
            <div style="background: var(--bg-card); border-radius: 12px; padding: 1.5rem; border: 1px solid var(--border);">
                {chart_items}
            </div>
        </section>

        <section aria-labelledby="detailed-findings-title">
            <h2 id="detailed-findings-title" class="section-title">Detailed Findings ({len(findings)})</h2>
